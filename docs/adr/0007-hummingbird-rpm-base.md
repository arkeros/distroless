...
Closed by adding a per-repo `repomd_signature` attribute to `rpm.install(...)` with two values:

- **`required`** (default) — every declared arch must serve a valid `.asc`. 404 or transport error aborts; tampered or wrong-key signature aborts. The right posture for any repo that actually publishes signatures (nginx.org's RHEL10 stable + mainline, which we verified empirically).
- **`optional`** — explicit opt-in for upstreams that don't publish `.asc`. A 404 on the signature URL warns loudly to stderr and continues with TLS-only trust at lock time *for that arch*; the lockfile is still written. A signature that is **present but invalid** (tampered payload, key outside the trust root) still hard-fails: the opt-out permits absence, not forgery. An attacker capable of forging a Hummingbird-key signature has already won at every other layer.

Hummingbird is set to `repomd_signature = "optional"` in [`//bazel/include:oci.MODULE.bazel`](../../bazel/include/oci.MODULE.bazel); nginx repos remain on the default. Two warning lines per pin run keep the degradation visible — no silent ride-through. If Hummingbird later starts publishing `repomd.xml.asc` the verification re-engages automatically; flipping back to `required` becomes a one-line ADR-amendment change.

Tests cover the four properties that matter under the new policy: `optional` tolerates 404, `required` rejects 404, `optional` still rejects a tampered/wrong-key signature, and the CLI parse rejects unknown policy values. Empirically: `bazel run @hummingbird//:pin` now refreshes the lockfile against the live upstream (rev + `repomd_sha256` rotate; per-package sha256 untouched in the snapshot tested); `bazel run @nginx_stable_rpm//:pin` continues to verify strictly. The surrounding cron's existing "only `repomd_sha256` changed → discard" gate absorbs the metadata-only churn.
...
## Amendment (2026-09-06): python migrated, and what its tests found in the base

Step 5's `python` shipped as `//images/python`: one image per line Hummingbird packages and upstream still patches (3.11, 3.12, 3.13, 3.14), each the distro's own `python3.NN` + `-libs` rpms on the cc base with every library the stdlib links — OpenSSL, SQLite, libffi, expat, xz, bzip2, mpdecimal, readline, ncurses, gdbm, libuuid, and per line libb2, libxcrypt or libzstd — as its own rpm, so the SBOM names each of them and the secdb routes their CVEs. python-build-standalone was considered for the lines Debian sid no longer carries and rejected: it folds those libraries into libpython, where no scanner sees them. The libraries every line shares are one layer, byte-identical across lines, so the registry stores that blob once.

Its structure tests are behavioural — import every extension module, open a TLS 1.3 connection and verify the chain, resolve a time zone — and three of them found defects in layers every Hummingbird image had been shipping:

**1. rpm-extract shipped hardlinked files empty.** rpm's cpio writes a hardlink set as one entry per path with a zero filesize and the payload once, on the last path. Copying entries one by one wrote 1113 of tzdata's 1801 zone files as empty, so `zoneinfo.ZoneInfo("Europe/Madrid")` failed with "Invalid TZif file"; java and node never noticed because each carries its own zone database. rpm-extract now holds a set's earlier paths until the payload arrives and writes them as tar hardlinks to it; a set of empty files, which never gets a payload entry, is written at end of stream. Locked by `TestExtract_HardlinksCarryContent` on the tzdata fixture and `TestWritePayloadAsTar_Hardlinks` on a hand-built cpio stream.

**2. No CA bundle behind `/etc/ssl/certs/ca-certificates.crt`.** §"Static composition" claimed Hummingbird's ca-certificates rpm ships the bundle at a Debian-compatible path. It ships the symlink; the file it points at, `/etc/pki/ca-trust/extracted/pem/tls-ca-bundle.pem`, is written by the rpm's `%post` script from the p11-kit trust source, and rpm-extract runs no scripts. Every TLS verification on a Hummingbird image failed with "unable to get local issuer certificate"; `cc_tls_test` runs on Debian only, so nothing said so. `@rules_rpm//rpm/tools/ca-bundle` now does the server-auth part of `trust extract`: anchors that are `trusted: true`, not `x-distrusted`, and whose extended-key-usage object, where p11-kit attaches one, allows server authentication — 146 of the 390 certificates in the source at amendment time — plus OpenSSL's two `/etc/pki/tls` default paths. `//images/common:cacerts_hummingbird` composes it into the static layer, and `python*_network_test` is the handshake that proves it.

**3. pin compared version-release as one string.** `rpmvercmp("2.42-7.1.hum1", "2.42.2-3.4.hum1")` reads 2.42.7.1 against 2.42.2.3.4 and prefers the older libuuid, which carried four fixed High CVEs the lockfile kept missing; audit-libs and systemd-libs were held back the same way. `compareEVR` now orders epoch, version and release as rpm does.
