...
| Several `vuln` (or `openvex`) attestations on one digest | Expected. `publish` adds a scan whenever the pinned grype database (or, at the same database, grype itself) is newer than every scan on the digest, and a VEX document whenever its statements changed; the rule is `//oci/publish:stale.jq`. The Directory shows the newest scan by `scanFinishedOn` and the newest VEX document by log time | Nothing. To prune, delete older referrers via the packages API (see GHCR notes); never the newest |
