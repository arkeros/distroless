...
def java_layers(major_version):
    ...
    def _layers(ctx):
        ...
        layers.append("//images/cc:cc_{}_hummingbird_layer".format(ctx.arch))
        if ctx.mode == "_debug":
            layers += [
                "//images/java:java_jdk_{}_{}_hummingbird_layer".format(major_version, ctx.arch),
                "//images/java:rpmdb_java_debug_{}_{}_hummingbird".format(major_version, ctx.arch),
            ]
        else:
            layers += [
                "//images/java:java_jre_{}_{}_hummingbird_layer".format(major_version, ctx.arch),
                "//images/java:rpmdb_java_{}_{}_hummingbird".format(major_version, ctx.arch),
            ]
        return layers
