...
The verification policy (OIDC issuer + workflow subject) is the single source of truth in [`//oci:cosign_policy.bzl`](../oci/cosign_policy.bzl). External consumers pin both `--certificate-oidc-issuer` and `--certificate-identity-regexp` so a signature minted from a different repo or workflow file is rejected.
